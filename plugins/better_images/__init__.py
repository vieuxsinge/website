"""
Modified version of the "better_images_and_figures" plugin form the pelican
plugins suite.
...
def content_object_init(instance):
    if instance.__class__ == contents.Page:
        return
    ...
    if instance._content is not None:
        ...
        for img in soup(['img', 'object']):
            ...
            if img_path.startswith(('{filename}', '|filename|')):
                ...
                error = ("Better Fig. Error: img_path should start with either"
                         " {filename}, |filename| or /static")
            ...
            img_path = os.path.sep.join([el for el in img_path.split("/")
                                         if len(el) > 0])
